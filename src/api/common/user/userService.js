...
class UserService {
  ...
  findByEmailOrPhone(data){
    return this.repository.findByEmailOrPhone(data);
  }

  findByMobile(phone_number) {
    return this.repository.findByMobile(phone_number);
  }
  ...
  mapUserToDto(user) {
    return user ? {
      ...
      internship_preferences: user.internship_preferences,
  ...
  editUserToken(token, userId) {
    return this.editToken(token, userId)
      .then(user => {
        return cipher.generateResponseTokens(user);
      })
      .catch(error => {
  ...
  editToken(dto, userId) {
    const user = this.mapUserToken(dto);
    return this.repository.editCurrentUserToken(userId, user);
  }
  ...
  Addsubscription(startdate,duration){
